...
@frappe.whitelist()
def stock_reconciliation_set_warehouse(doctype, txt, searchfield, start, page_len, filters):
    warehouse_list = []
    
    company = filters.get('company') if filters else None
    
    if frappe.db.exists("Warehouse Restriction", {'user': frappe.session.user}):
        r_warehouse_list = frappe.get_list("Warehouse Restriction", {'user': frappe.session.user, 'document_name': "Stock Reconciliation"}, ['name'])
...
def restrictwarehouse(doc,method=None):
    ...
    if frappe.db.exists("Warehouse Restriction", {'user': frappe.session.user,'document_name': "Stock Reconciliation"}):
        
        restricted_warehouse = frappe.get_doc("Warehouse Restriction", frappe.session.user+"-Stock Reconciliation") 
        exist = 0
